...
var audienceClient = AgoraRTC.createClient({mode: 'live', codec: 'vp8'});  // h264 better detail at a higher motion
...
// init Agora SDK
audienceClient.init(agoraAppId, function () {
...
// client callbacks
audienceClient.on('stream-published', function (evt) {
  console.log('Publish local stream successfully');
});

// when a remote stream is added
audienceClient.on('stream-added', function (evt) {
  console.log('new stream added: ' + evt.stream.getId());
});

audienceClient.on('stream-removed', function (evt) {
...
audienceClient.on('liveStreamingStarted', function (evt) {
  console.log("Live streaming started");
}); 

audienceClient.on('liveStreamingFailed', function (evt) {
  console.log("Live streaming failed");
}); 

audienceClient.on('liveStreamingStopped', function (evt) {
  console.log("Live streaming stopped");
});

audienceClient.on('liveTranscodingUpdated', function (evt) {
  console.log("Live streaming updated");
}); 

// ingested live stream 
audienceClient.on('streamInjectedStatus', function (evt) {
  console.log("Injected Steram Status Updated");
  console.log(JSON.stringify(evt));
}); 

// when a remote stream leaves the channel
audienceClient.on('peer-leave', function(evt) {
  console.log('Remote stream has left the channel: ' + evt.stream.getId());
});

// show mute icon whenever a remote has muted their mic
audienceClient.on('mute-audio', function (evt) {
  console.log('Mute Audio for: ' + evt.uid);
});

audienceClient.on('unmute-audio', function (evt) {
  console.log('Unmute Audio for: ' + evt.uid);
});

// show user icon whenever a remote has disabled their video
audienceClient.on('mute-video', function (evt) {
  console.log('Mute Video for: ' + evt.uid);
});

audienceClient.on('unmute-video', function (evt) {
...
function joinChannel() {
  ...
  audienceClient.join(token, channelName, userID, function(uid) {
...
function createCameraStream(uid, deviceIds) {
  ...
  localStream.init(function() {
    ...
    audienceClient.publish(localStream, function (err) {
...
function leaveChannel() {
  ...
  audienceClient.leave(function() {
    console.log('client leaves channel');
    localStreams.camera.stream.stop() // stop the camera stream playback
    localStreams.camera.stream.close(); // clean up and close the camera stream
    audienceClient.unpublish(localStreams.camera.stream); // unpublish the camera stream
...
function getCameraDevices() {
  ...
  audienceClient.getCameras (function(cameras) {
...
function getMicDevices() {
  ...
  audienceClient.getRecordingDevices(function(mics) {
...
function startLiveTranscoding() {
  ...
  audienceClient.setLiveTranscoding(configRtmp);
  if(rtmpUrl !== '') {
    audienceClient.startLiveStreaming(rtmpUrl, true)
...
function addExternalSource() {
  ...
  // set live transcoding config
  audienceClient.addInjectStreamUrl(externalUrl, injectStreamConfig)
...
function addExternalTransmitionMiniView(rtmpUrl){
  ...
  $('#rtmp-toggle').click(function() {
    if ($(this).hasClass('pulse-anim')) {
      audienceClient.stopLiveStreaming(externalBroadcastUrl)
    } else {
      audienceClient.startLiveStreaming(externalBroadcastUrl, true)
    }
