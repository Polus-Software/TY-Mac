...
<html lang="en">
<!-- appid : 88a4d4d0cb874afd82ada960cdcc1b1f -->
<!-- channel : Session -->
<!-- Token : 00688a4d4d0cb874afd82ada960cdcc1b1fIADQgotV+/DvrAc2oMVLXV2oZuD6qWvn94s4s38r+e7bQUjs+R8AAAAAEAD3dRUDBVqLYQEAAQAEWoth -->
<head>
  <meta charset="UTF-8">
...
<body class="agora-theme">
  <div class="navbar-fixed">
    <nav class="agora-navbar">
      <div class="nav-wrapper valign-wrapper" style="background-color:white;">
        <h5 class="left-align" style="color:#000;">TY-Mac Session</h5>
  ...
  <form id="form" class="row col l12 s12">
    <div class="row container col l12 s12">
      <div class="col" style="min-width: 433px; max-width: 443px; visibility:hidden;">
        <div class="card" style="margin-top: 0px; margin-bottom: 0px;">
          <div class="row card-content" style="margin-bottom: 0px;">
              <div class="input-field">
                <label for="appID" class="active">App ID</label>
                <input type="text" placeholder="App ID" name="appID" value="88a4d4d0cb874afd82ada960cdcc1b1f">
              </div>
              <div class="input-field">
                <label for="channel" class="active">Channel</label>
                <input type="text" placeholder="channel" name="channel" value="Session">
              </div>
              <div class="input-field">
                <label for="token" class="active">Token</label>
                <input type="text" placeholder="token" name="token" value="00688a4d4d0cb874afd82ada960cdcc1b1fIADQgotV+/DvrAc2oMVLXV2oZuD6qWvn94s4s38r+e7bQUjs+R8AAAAAEAD3dRUDBVqLYQEAAQAEWoth">
              </div>
              <div class="row" style="margin: 0">
                <div class="col s12">
                <button class="btn btn-raised btn-primary waves-effect waves-light" id="join">JOIN</button>
                <button class="visible-btn btn btn-raised btn-primary waves-effect waves-light" id="leave" style="visibility:visible;background-color: #fff !important;color: #000 !important;box-shadow: none;border: 1px solid;border-radius: 3px;">Leave</button>
                @if($userType == 'instructor')
                <button share="" type="button" class="visible-btn btn btn-raised btn-primary waves-effect waves-light" id="screen-share" style="visibility:visible;background-color: #fff !important;color: #000 !important;box-shadow: none;border: 1px solid;border-radius: 3px;">Share screen</button>
                @else
                <button share="" type="button" class="btn btn-raised btn-primary waves-effect waves-light" id="screen-share" style="visibility:hidden;background-color: #fff !important;color: #000 !important;box-shadow: none;border: 1px solid;border-radius: 3px;">Share screen</button>
                @endif
  ...
  <script src="{{ asset('/assets/AgoraRTCSDK-3.6.5.js') }}"></script>
  <script>
    // window.addEventListener('load', function (event) {
    //   document.getElementById('join').click();
    // });

document.getElementById('screen-share').addEventListener('click', function (event) {
       if(document.getElementById('screen-share').getAttribute("share") == "") {
        document.getElementById('screen-share').setAttribute("share", "active")
        var screenShare = 1;
    ...
    setTimeout(function() {
        if (validator(params, fields)) {
          join(rtc, params, screenShare)
        }
    },1000);
    document.getElementById('screen-share').innerHTML = "STOP SCREEN SHARING";
       } else {
        document.getElementById('screen-share').setAttribute("share", "")
      var fields = ["appID", "channel"]
      var params = serializeformData();
      if (validator(params, fields)) {
          leave(rtc)
    ...
    document.getElementById('screen-share').innerHTML = "SHARE SCREEN";
       }
    ...
    /**
      ...
      * }
     **/
    function join (rtc, option, screenShare = 0) {
