...
@endif
  <input id="topic_id" type="hidden" value="{{ $topicId }}" />
  ...
  <script type="text/javascript">
    ...
    $('#offcanvasClose').click(function(){
      ...
      $('#offcanvasBottom').css('visibility', 'hidden');
        ...
        let path = "/generate-token-1-on-1/" + topic + "/" + user;
        fetch(path, {
            method: 'GET',
            headers: {
...
document.getElementById('chat_box').addEventListener('keyup', function(e) {
  ...
  let topic = document.getElementById('topic_id').value;
  let student = document.getElementById('user_id').value;
  message = message.trim();
    if(e.which == 13 && message != "") {
      let message = document.getElementById('chat_box').value;
      document.getElementById('chat_box').value = "";
      
      let path = "{{ route('save-single-session-chat') }}?topic=" + topic + "&message=" + message + "&student=" + student;
      fetch(path, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
...
$(document).ready(function(){
  ...
  setInterval(function() {
      ...
      topic = document.getElementById('topic_id').value
      student = document.getElementById('user_id').value
      let path = "{{ route('get-session-attendance-list') }}?topic=" + topic + "&student=" + student;
        fetch(path, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
      ...
      let chatPath = "{{ route('get-single-session-chat') }}?topic=" + topic + "&student=" + student;
      fetch(chatPath, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
...
$(document).on('click', '.cabinet-item', function(e) {
    $('.screen-share-player-container').appendTo('.teacher-1v1');
    $('#stop_sharing').removeClass('nodisplay');

    let topicContentId = document.getElementById('topic_id').value;
    let student = document.getElementById('user_id').value;
    let path = "{{ route('start-screen-share') }}?topicId=" + topicContentId + "&student=" + student;
      fetch(path, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
...
setInterval(function () {
    let topicId = document.getElementById('topic_id').value;
    let student = document.getElementById('user_id').value;
    let path = "{{ route('get-session-push-record') }}?topicId=" + topicId + "&student=" + student;
    let userType = document.getElementById('user_type').value;
    fetch(path, {
      method: 'POST',
      headers: {
      ...
      if(userType == 'student') {
          ...
          if((data.is_presenting == 1 || data.is_presenting == "1") && presentingFlag == 0) {
            presentingFlag = 1;
            $('#course_content_iframe').appendTo('.teacher-1v1');
            document.getElementById('course_content_iframe').classList.remove('nodisplay');
            document.getElementById('course_content_iframe').setAttribute('src', 'https://view.officeapps.live.com/op/embed.aspx?src=' + data.docUrl);
          } else {
...
for(index = 0; index < length;index++) {
    ...
    contentEle[index].addEventListener('click', function(event) {
      ...
      let path = "{{ route('push-session-live-record') }}?topicId=" + topicContentId + "&student=" + student;
      document.getElementById('close_content').classList.remove('nodisplay');
...
document.getElementById('close_content').addEventListener('click', function(e) {
    ...
    let student = document.getElementById('user_id').value;
      let path = "{{ route('stop-content-presenting') }}?topicId=" + topicContentId + "&student=" + student;
      fetch(path, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
