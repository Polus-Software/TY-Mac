...
class AdminController extends Controller
{
    ...
    public function courseSearch(Request $request) {
        ...
        if($searchTerm === 'all') {
            ...
            $courses = Course::where('course_title', 'LIKE', '%' . $searchTerm . '%')
        ...
        return view('Student.allCourses', ['courseDatas' => $courseDetailsObj, 'allCourseCategory' => $allCourseCategory, 'filters' => $filters, 'instructors' => $instructors, 'searchTerm' => $searchTerm, 'noFiltersFlag' => $allFiltersFlag]);
    ...
    public function saveAdmin(Request $request) {
        ...
        $validatedData = $request->validate([
            ...
            'password' => 'required|min:5|max:12|regex:/^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[\d\x])(?=.*[!-_:$#%]).*$/',
            'timezone' => 'required'
        ]);
        $userType = UserType::where('user_role', 'admin')->value('id');
        $admin = new User;
    ...
    public function updateAdmin(Request $request) {
        ...
        $validatedData = $request->validate([
            ...
            'password' => 'nullable|min:5|max:12|regex:/^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[\d\x])(?=.*[!-_:$#%]).*$/',
        ]);
            ...
            if ($admin) {
                ...
                if(count($admins)) {
                    foreach($admins as $admin) {
                        ...
                        $html = $html . '<td class="align-middle"><img src="/storage/images/'. $admin->image .'" class="rounded-circle" alt="" style="width:40px; height:40px;"></td>';
