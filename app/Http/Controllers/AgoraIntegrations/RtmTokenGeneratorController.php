...
class RtmTokenGeneratorController extends Controller
{
        ...
        if($occurrences == "Daily") {
            ...
            while($date <= $endDate) {
                if(isset($topics[$topicsCounter])) {
                    ...
                    Mail::to('infosmtp')->to($instructorEmail)->send(new InstructorMailAfterLiveSessionScheduled($details));
                    ...
                    foreach($students as $student){
                        ...
                        Mail::to('infosmtp')->to($studentEmail)->send(new MailAfterLiveSessionScheduled($data));
    ...
    public function studentExitAfterFeedback($sessionId, $newTime) {
        ...
        if($user) {
            ...
            Mail::to('infosmtp')->to($user->email)->send(new RecommendationMail($recDetails));
            
            Mail::to('infosmtp')->to($instructorObj->value('email'))->send(new RecommendationMailInstructor($recDetails));
    ...
    public function studentExit(Request $request) {
        ...
        if($user) {
            ...
            if($percent >= 95){
                ...
                Mail::to('infosmtp')->to($studentEmail)->send(new mailAfterCourseCompletion($mailData));
    ...
    public function buildToken1v1(Request $request, $topic, $user) {
        ...
        if($userObj->role_id == 2) {
            ...
            Mail::to('infosmtp')->to($studentEmail)->send(new OneOnOneMail($details));
        }
