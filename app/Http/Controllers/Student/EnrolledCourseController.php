...
// Student achievements
    ...
    public function submitAssignment(Request $request){
        ...
        $data= [
         ...
         Mail::to('infosmtp')->to($instructorEmail)->send(new MailAfterAssignmentSubmission($data));
    ...
    public function saveGeneralChat(Request $request) {
        ...
        if($loggedInId == $studentId) {
            ...
            $generalChat->read_by_instructor = false;
             Mail::to('infosmtp')->to($instructorEmail)->send(new GeneralChatInstructorMail($details));
        } else if($loggedInId == $instructorId) {
            $generalChat->read = false;
            $generalChat->read_by_instructor = true;    
             Mail::to('infosmtp')->to($studentEmail)->send(new GeneralChatStudentMail($details));
        }
