...
class CoursesCatalogController extends Controller
{
    ...
    public function showCourse($id, Request $request) {
        if ($request->session()->exists('isEnrollNow')) {
            $isEnrollNow = $request->session()->get('isEnrollNow');
            if($isEnrollNow == 'true') {    
                 $request->session()->forget('isEnrollNow');
                return redirect('/register-course?id='.$id);
            }
        }
    ...
    public function enrollCourse(Request $request){
        if (Auth::check() == true) {
            ...
            $request->session()->put('isEnrollNow', 'true');
    ...
    try {
       ...
       $mailDetails = [
        ...
        Mail::to('infosmtp')->to($studentEmail)->send(new StudentMailAfterEnrolling($mailDetails));
        ...
        Mail::to('infosmtp')->to($instructorEmail)->send(new InstructorMailAfterEnrolling($data));
        ...
        foreach($admins as $admin) {
            ...
            Mail::to('infosmtp')->to($admin->email)->send(new AdminMailAfterStudentEnrolling($mailData));
    ...
    if($search !== 'all') {
        $courses = $courses->where('course_title', 'LIKE', '%' . $search . '%');
    }
    ...
    public function courseDropDown(Request $request) {
        ...
        $courses = DB::table('courses')->where('is_published', true);
        if($filterValue == "all") {
            $courses = $courses->where('is_published', true);
        } elseif ($filterValue == "most_popular") {
            $courses = $courses->orderBy('enrollments', 'DESC');
        } elseif ($filterValue == "most_reviewed") {
            $courses = $courses->orderBy('ratings_count', 'DESC');
        }
        if($search !== 'all') {
            $courses = $courses->where('course_title', 'LIKE', '%' . $search . '%');
        }
    ...
    public function haveAnyQuestion(Request $request) {
        ...
        try {
            ...
            Mail::to('infosmtp')->to('info@thinklit.com')->send(new InstructorMailAfterStudentConcern($details));
            Mail::to('infosmtp')->to($instructorEmail)->send(new InstructorMailAfterStudentConcern($details));
