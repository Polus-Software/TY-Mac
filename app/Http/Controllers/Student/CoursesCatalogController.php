...
class CoursesCatalogController extends Controller
{
    ...
    public function haveAnyQuestion(Request $request) {
        ...
        try {
            ...
            $email = $request->email;
            $courseId = $request->course_id;
            $assigned = DB::table('assigned_courses')->where('course_id', (int)$courseId)->value('user_id');
            $user = User::where('id', $assigned);
            $instructorName = $user->value('firstname').' '.$user->value('lastname');
            $instructorEmail = $user->value('email');
