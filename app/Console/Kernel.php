                        ...
                        if($liveSession->start_date == $threeDays) {
                            if($liveSession->first_notification_sent == 0) {
                                ...
                                foreach($enrolledCourseData as $data) {
                                    ...
                                    Mail::to('infosmtp')->to($student->value('email'))->send(new LiveSessionReminderMail($details));
                                ...
                                Mail::to('infosmtp')->to($instructor->value('email'))->send(new LiveSessionReminderInstructor($iDetails));
                            }
                        }
                ...
                if($assignment->notification_sent == 0 || $assignment->notification_sent == null){
                    ...
                    foreach($enrolledCourseData as $data) { 
                        ...
                        if($assignmentFlag == true && $assignment->due_date == $oneDay) {
                            Mail::to('infosmtp')->to($student->value('email'))->send(new AssignmentReminder($data));
